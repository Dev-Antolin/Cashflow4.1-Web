...
Public Class Form1
    ...
    Private Sub calldate()
        ...
        transdate = Format(CDate(DateTime.Today), "yyyy-MM-dd")
        transmonth = Format(CDate(DateTime.Today), "MM")
        transyear = Format(CDate(DateTime.Today), "yyyy")
    End Sub
    ...
    Private Sub insert_to_CF_vismin_wide()
        ...
        'code starts here-------------------------------------------------------------------
        Dim ls_begbal As String = "select isnull (sum(beginningbalance),0) from CF_region_vismin where " & _
        " transdate = '" + transdate + "'"
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_endingbal As String = "select isnull (sum(EndingBalance),0) from CF_region_vismin where " & _
          " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_foodproducts As String = "select isnull (sum(Foodproducts),0) from CF_region_vismin where " & _
          " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_insurance As String = "select isnull (sum(Insurance),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------jen 
        Dim ls_outrightsales As String = "select isnull(sum(outrightsales),0) from CF_region_vismin where transdate =  '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '--------------------------------------------------------------
        Dim ls_layaway As String = "select isnull(sum(layaway),0) from CF_region_vismin where transdate =  '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '--------------------------------------------------------------
        Dim ls_salesreturn As String = "select isnull(sum(salesreturn),0) from CF_region_vismin where transdate =  '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '--------------------------------------------------------------
        Dim ls_layawaycancel As String = "select isnull(sum(layawaycancel),0) from CF_region_vismin where transdate =  '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        Dim ls_interest As String = "select isnull (sum(interest),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_kppayout As String = "select isnull (sum(kp_payout),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_kpsendout As String = "select isnull (sum(kp_sendout),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_kpsendoutcomm As String = "select isnull (sum(KP_Sendout_Comm),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_lukat As String = "select isnull (sum(Lukat),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_otherincome As String = "select isnull (sum(OtherIncome),0) from CF_region_vismin where " & _
    " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_Prenda As String = "select sum(prenda) from CF_region_vismin where " & _
      " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_telecomms As String = "select isnull (sum(telecomms),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_souvenirs As String = "select isnull (sum(Souvenirs),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_Corp_Sendout As String = "select isnull (sum(Corp_Sendout),0) from CF_region_vismin where " & _
      " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_Corp_Payout As String = "select isnull (sum(Corp_Payout),0) from CF_region_vismin where " & _
      " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_Corp_Comm As String = "select isnull (sum(Corp_Comm),0) from CF_region_vismin where " & _
   " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_WesternUnionComm As String = "select isnull (sum(WesternUnionComm),0) from CF_region_vismin where " & _
   " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_WesternUnionPayout As String = "select isnull (sum(WesternUnionPayout),0) from CF_region_vismin where " & _
   " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
  ...
  " transdate = '" + transdate + "'"
        ...
        'c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_FundTransferDebit As String = "select isnull (sum(FundTransferDebit),0) from CF_region_vismin where " & _
   " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_FundTransferCredit As String = "select isnull (sum(FundTransferCredit),0) from CF_region_vismin where " & _
      " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        c.DisposeR()
        '--------------------------------------------------------------
        Dim ls_BranchExpense As String = "select isnull (sum(BranchExpense),0) from CF_region_vismin where " & _
      " transdate = '" + transdate + "'"
        ...
        c.DisposeR()
        '----------------------------------------------------------
        Dim ls_OtherExpense As String = "select isnull(sum(OtherExpense),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '/-added last 2/9/2011
        '----------------------------------------------------------
        Dim ls_nso As String = "select isnull (sum(NSO),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_mccr As String = "select isnull (sum(MCCashReceipts),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_mccd As String = "select isnull (sum(MCCashDisbursements),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_racr As String = "select isnull (sum(RACashReceipts),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_racd As String = "select isnull (sum(RACashDisbursements),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_dfb As String = "select isnull (sum(DepositFromBank),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_wfb As String = "select isnull (sum(WithdrawalFromBank),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_rts As String = "select isnull (sum(returntosender),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        Dim ls_cashover As String = "select isnull(sum(cashover),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        '----------------------------------------------------------
        '----------------------------------------------------------
        Dim ls_cashshort As String = "select isnull(sum(cashshort),0) from CF_region_vismin where " & _
       " transdate = '" + transdate + "'"
        If c.ErrorConnectionReading(False) Then Exit Sub
        ...
        If gb_vismin_update = True Then
           ...
           " [BranchExpense]=" & ld_branchexpense_lv & ",[OtherExpense] = " & ld_otherexpense_lv & " ,[NSO] = " & ld_nso_lv & " ,[MCCashReceipts] = " & ld_MCCR_lv & ",[MCCashDisbursements] = " & ld_MCCD_lv & ",[RACashReceipts] = " & ld_RACR_lv & ",[RACashDisbursements] = " & ld_RACD_lv & ",[DepositFromBank] = " & ld_Depositfrombank_lv & " ,[WithdrawalFromBank] = " & ld_withdrawalfrombank_lv & " ,[ReturnToSender] = " & ld_returntosender & " ,[cashover] = " & ld_CashOver & " ,[cashshort] = " & ld_CashShort & "  ,[dategenerated] = '" + Now.Date + "'         WHERE  transdate = '" + transdate + "' "
            ...
            '\ email functionality added 08-30-2012
        Else
            Dim ls_s As String = "INSERT INTO CF_vismin_wide ([Id],[BeginningBalance], [EndingBalance], [FoodProducts],   [Insurance],  [outrightsales],  [layaway],  [salesreturn],  [layawaycancel],  [Interest], [KP_Payout], [KP_Sendout], [KP_Sendout_Comm], [Lukat], [OtherIncome],[Prenda], [Telecomms], [Souvenirs], [Corp_Sendout], [Corp_Payout], [Corp_Comm], [WesternUnionComm],[WesternUnionPayout],[WesternUnionSendout],[FundTransferDebit], [FundTransferCredit], [BranchExpense], [OtherExpense],[NSO],[MCCashReceipts],[MCCashDisbursements],[RACashReceipts],[RACashDisbursements],[DepositFromBank],[WithdrawalFromBank],[returntosender],[cashover],[cashshort],[Transdate], [DateGenerated])" & _
                 ...
                 " " & ld_otherexpense_lv & "," & ld_nso_lv & "," & ld_MCCR_lv & "," & ld_MCCD_lv & "," & ld_RACR_lv & "," & ld_RACD_lv & "," & ld_Depositfrombank_lv & "," & ld_withdrawalfrombank_lv & "," & ld_returntosender & ", " & ld_CashOver_lv & " , " & ld_CashShort_lv & "  ,'" + transdate + "','" + Now.Date + "') "
    ...
    Private Sub insert_to_CF_region_vismin()
        ...
        For li_i = 0 To li_listviewcount
            ...
            'code starts here-------------------------------------------------------------------
            Dim ls_begbal As String = "select isnull(sum(beginningbalance),0) from CF_area_vismin where " & _
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_endingbal As String = "select isnull (sum(EndingBalance),0) from CF_area_vismin where " & _
              "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_endingbal, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_foodproducts As String = "select isnull (sum(Foodproducts),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_foodproducts, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_insurance As String = "select isnull (sum(Insurance),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_insurance, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------jen
            Dim ls_outrightsales As String = "select isnull (sum(outrightsales),0) from CF_area_vismin where transdate =  '" + transdate + " 'and  class_03 = '" + ls_class03 + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_outrightsales, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            Dim ls_layaway As String = "select isnull (sum(layaway),0) from CF_area_vismin where transdate =  '" + transdate + " 'and  class_03 = '" + ls_class03 + "'"
            ...
            Dim ls_salesreturn As String = "select isnull (sum(salesreturn),0) from CF_area_vismin where transdate =  '" + transdate + " 'and  class_03 = '" + ls_class03 + "'"
            ...
            '----------------------------------------------------------------------------------------------------
            Dim ls_layawaycancel As String = "select isnull (sum(layawaycancel),0) from CF_area_vismin where transdate =  '" + transdate + " 'and  class_03 = '" + ls_class03 + "'"
            ...
            Dim ls_interest As String = "select isnull (sum(interest),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_interest, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_kppayout As String = "select isnull (sum(kp_payout),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_kppayout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_kpsendout As String = "select isnull (sum(kp_sendout),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_kpsendout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_kpsendoutcomm As String = "select isnull (sum(KP_Sendout_Comm),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_kpsendoutcomm, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_lukat As String = "select isnull (sum(Lukat),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_lukat, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_otherincome As String = "select isnull (sum(OtherIncome),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_otherincome, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_Prenda As String = "select isnull (sum(prenda),0) from CF_area_vismin where " & _
      "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Prenda, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_telecomms As String = "select isnull (sum(telecomms),0) from CF_area_vismin where " & _
         "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_telecomms, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_souvenirs As String = "select isnull (sum(Souvenirs),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_souvenirs, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_Corp_Sendout As String = "select isnull (sum(Corp_Sendout),0) from CF_area_vismin where " & _
           "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Corp_Sendout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_Corp_Payout As String = "select isnull (sum(Corp_Payout),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Corp_Payout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_Corp_Comm As String = "select isnull (sum(Corp_Comm),0) from CF_area_vismin where " & _
      "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Corp_Comm, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_WesternUnionComm As String = "select isnull (sum(WesternUnionComm),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WesternUnionComm, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_WesternUnionPayout As String = "select isnull (sum(WesternUnionPayout),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WesternUnionPayout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_WesternUnionSendout As String = "select isnull (sum(WesternUnionSendout),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WesternUnionSendout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_FundTransferDebit As String = "select isnull (sum(FundTransferDebit),0) from CF_area_vismin where " & _
            "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_FundTransferDebit, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_FundTransferCredit As String = "select isnull (sum(FundTransferCredit),0) from CF_area_vismin where " & _
           "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_FundTransferCredit, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_BranchExpense As String = "select isnull (sum(BranchExpense),0) from CF_area_vismin where " & _
     "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_BranchExpense, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_OtherExpense As String = "select isnull (sum(OtherExpense),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_OtherExpense, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '/-added last 2/9/2011
            '----------------------------------------------------------
            Dim ls_NSO As String = "select isnull (sum(NSO),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_NSO, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_MCCR As String = "select isnull (sum(MCCashReceipts),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_MCCR, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_MCCD As String = "select isnull (sum(MCCashDisbursements),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_MCCD, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_RACR As String = "select isnull (sum(RACashReceipts),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_RACR, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_RACD As String = "select isnull (sum(RACashDisbursements),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_RACD, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_DFB As String = "select  isnull (sum(DepositFromBank),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_DFB, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_WFB As String = "select isnull (sum(WithdrawalFromBank),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WFB, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_rts As String = "select isnull (sum(returntosender),0) from CF_area_vismin where " & _
        "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_rts, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '\-added last 2/9/2011
            '/-added last 7/26/2011
            Dim ls_cashover As String = "select isnull (sum(cashover),0) from CF_area_vismin where " & _
   "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_cashover, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            Dim ls_cashshort As String = "select isnull (sum(cashshort),0) from CF_area_vismin where " & _
   "class_03 = '" + ls_class03 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_cashshort, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            If gb_region_update = True Then
                ...
                " [Telecomms] = " & ld_telecomms_lv & ", [Souvenirs]=" & ld_souvenirs_lv & ", [Corp_Sendout]=" & ld_corpsendout_lv & ", [Corp_Payout]=" & ld_corppayout_lv & ",[Corp_Comm]=" & ld_corpcomm_lv & " ,[WesternUnionComm]=" & ld_westernUnionComm_lv & ", " & _
                " [WesternUnionPayout]=" & ld_westernunionPayout_lv & ", [WesternUnionSendout]=" & ld_westernunionsendout_lv & ", [FundTransferDebit]=" & ld_fundtransferdebit_lv & ", [FundTransferCredit]=" & ld_fundtransfercredit_lv & ", " & _
                " [BranchExpense]=" & ld_branchexpense_lv & ",[OtherExpense] = " & ld_otherexpense_lv & " ,[NSO] = " & ld_nso_lv & " ,[MCCashReceipts] = " & ld_MCCR_lv & ",[MCCashDisbursements] = " & ld_MCCD_lv & ",[RACashReceipts] = " & ld_RACR_lv & ",[RACashDisbursements] = " & ld_RACD_lv & ",[DepositFromBank] = " & ld_Depositfrombank_lv & " ,[WithdrawalFromBank] = " & ld_withdrawalfrombank_lv & ",[returntosender] = " & ld_returntosender_lv & "  ,[cashover] = " & ld_cashover_lv & "  ,[cashshort] = " & ld_cashshort_lv & "  ,[dategenerated] = '" + Now.Date + "' WHERE  transdate = '" + transdate + "' and class_03 = '" + class03 + "'"
            ...
            Else
                Dim ls_s As String = "INSERT INTO CF_region_vismin ([Id],[Class_03], [BeginningBalance], [EndingBalance], [FoodProducts],   [Insurance] ,  [outrightsales],  [layaway],  [salesreturn],  [layawaycancel], [Interest], [KP_Payout], [KP_Sendout], [KP_Sendout_Comm], [Lukat], [OtherIncome],[Prenda], [Telecomms], [Souvenirs], [Corp_Sendout], [Corp_Payout], [Corp_Comm], [WesternUnionComm],[WesternUnionPayout],[WesternUnionSendout],[FundTransferDebit], [FundTransferCredit], [BranchExpense], [OtherExpense],[NSO],[MCCashReceipts],[MCCashDisbursements],[RACashReceipts],[RACashDisbursements],[DepositFromBank],[WithdrawalFromBank],[returntosender],[cashover],[cashshort],[Transdate], [DateGenerated])" & _
                     ...
                     " " & ld_corppayout_lv & ", " & ld_corpcomm_lv & ", " & ld_westernUnionComm_lv & ", " & _
                     " " & ld_westernunionPayout_lv & "," & ld_westernunionsendout_lv & "," & ld_fundtransferdebit_lv & "," & ld_fundtransfercredit_lv & "," & ld_branchexpense_lv & ", " & _
                     " " & ld_otherexpense_lv & " ," & ld_nso_lv & "," & ld_MCCR_lv & "," & ld_MCCD_lv & "," & ld_RACR_lv & "," & ld_RACD_lv & "," & ld_Depositfrombank_lv & "," & ld_withdrawalfrombank_lv & "," & ld_returntosender_lv & "," & ld_cashover_lv & ", " & ld_cashshort_lv & "  ,'" + transdate + "','" + Date.Now + "')                 "
...
" where branchcode = '" + bcode + "' and transdate ='" + transdate + "'"
            ...
            '" where branchcode = '" + bcode + "' and transdate ='" + transdate + "'"
            Dim ls_mccd As String = "select sum(totalcashdisbursements * -1) from (select * from vwCashFlowMCCashDisbursement_CF_ver3 where transdate = '" + transdate + "' and branchcode = '" + bcode + "')x where totalcashdisbursements < 0"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_mccd, rdr, sqlmsg) Then
                If rdr.Read Then
                    'ld_otherincome = Trim(rdr(0)) 'area otherincome
         ...
         " where branchcode = '" + bcode + "' and transdate ='" + transdate + "'"
            ...
            Try
                If gb_pb_update = True Then
                    ...
                    " [Telecomms] = " & ld_telecomms_lv & ", [Souvenirs]=" & ld_souvenirs_lv & ", [Corp_Sendout]=" & ld_corpsendout_lv & ", [Corp_Payout]=" & ld_corppayout_lv & ",[Corp_Comm]=" & ld_corpcomm_lv & " ,[WesternUnionComm]=" & ld_westernUnionComm_lv & ", " & _
                    " [WesternUnionPayout]=" & ld_westernunionPayout_lv & ", [WesternUnionSendout]=" & ld_westernunionsendout_lv & ", [FundTransferDebit]=" & ld_fundtransferdebit_lv & ", [FundTransferCredit]=" & ld_fundtransfercredit_lv & ", " & _
                    " [BranchExpense]=" & ld_branchexpense_lv & ",[OtherExpense] = " & ld_otherexpense_lv & " ,[NSO] = " & ld_nso_lv & " ,[MCCashReceipts] = " & ld_MCCR_lv & ",[MCCashDisbursements] = " & ld_MCCD_lv & ",[RACashReceipts] = " & ld_RACR_lv & ",[RACashDisbursements] = " & ld_RACD_lv & ",[DepositFromBank] = " & ld_Depositfrombank_lv & " ,[WithdrawalFromBank] = " & ld_withdrawalfrombank_lv & ",[ReturnToSender] = " & ld_returntosender_lv & " , [Cashover] = " & ld_CashOver_lv & " , [cashshort] = " & ld_CashShort_lv & "  ,[dategenerated] = '" + Now.Date + "' WHERE branchcode = '" + bcode + "' and transdate = '" + transdate + "' "
                    ...
                    c26.DisposeR()
                Else
                    Dim ls_s As String = "INSERT INTO CF_PB_vismin ([Id],[branchcode],[branchname],[Class_03], [Class_04], [BeginningBalance], [EndingBalance], [FoodProducts],   [Insurance],  [outrightsales],  [layaway],  [salesreturn],  [layawaycancel], [Interest], [KP_Payout], [KP_Sendout], [KP_Sendout_Comm], [Lukat], [OtherIncome],[Prenda], [Telecomms], [Souvenirs], [Corp_Sendout], [Corp_Payout], [Corp_Comm], [WesternUnionComm],[WesternUnionPayout],[WesternUnionSendout],[FundTransferDebit], [FundTransferCredit], [BranchExpense], [OtherExpense],[NSO],[MCCashReceipts],[MCCashDisbursements],[RACashReceipts],[RACashDisbursements],[DepositFromBank],[WithdrawalFromBank],[ReturnToSender],[cashover],[cashshort] ,[Transdate], [DateGenerated])" & _
                        ...
                        " " & ld_corppayout_lv & ", " & ld_corpcomm_lv & ", " & ld_westernUnionComm_lv & ", " & _
                        " " & ld_westernunionPayout_lv & "," & ld_westernunionsendout_lv & "," & ld_fundtransferdebit_lv & "," & ld_fundtransfercredit_lv & "," & ld_branchexpense_lv & ", " & _
                        " " & ld_otherexpense_lv & "," & ld_nso_lv & "," & ld_MCCR_lv & "," & ld_MCCD_lv & "," & ld_RACR_lv & "," & ld_RACD_lv & "," & ld_Depositfrombank_lv & "," & ld_withdrawalfrombank_lv & "," & ld_returntosender_lv & ", " & ld_CashOver_lv & " ," & ld_CashShort_lv & " ,'" + transdate + "','" + Now.Date + "') "
    ...
    Private Sub insert_to_CF_area_vismin()
        ...
        For li_i = 0 To li_listviewcount
            ...
            'code starts here-------------------------------------------------------------------
            Dim ls_begbal As String = "select isnull (sum(beginningbalance),0) from CF_pb_vismin where " & _
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_endingbal As String = "select isnull (sum(EndingBalance),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_endingbal, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_foodproducts As String = "select isnull (sum(Foodproducts),0) from CF_pb_vismin where " & _
           "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_foodproducts, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_insurance As String = "select isnull (sum(Insurance),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_insurance, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            Dim ls_outrightsales As String = "select isnull (sum(outrightsales),0) from CF_pb_vismin where transdate =  '" + transdate + "'and class_04 = '" + ls_class04 + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_outrightsales, rdr, sqlmsg) Then
                If rdr.Read Then
                    ld_outrightsales = Trim(rdr(0)) 'area outrightsales
            ...
            c.DisposeR()
            '-------------------------------- 
            Dim ls_layaway As String = "select isnull (sum(layaway),0) from CF_pb_vismin where transdate =  '" + transdate + "'and class_04 = '" + ls_class04 + "'"
            ...
            '--------------------------------
            Dim ls_salesreturn As String = "select isnull (sum(salesreturn),0) from CF_pb_vismin where transdate =  '" + transdate + "'and class_04 = '" + ls_class04 + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_salesreturn, rdr, sqlmsg) Then
                If rdr.Read Then
                    ld_salesreturn = Trim(rdr(0)) 'area salesreturn
            ...
            '----------------------------------------------------
            Dim ls_layawaycancel As String = "select isnull (sum(layawaycancel),0) from CF_pb_vismin  where transdate =  '" + transdate + "'and class_04 = '" + ls_class04 + "'"
            ...
            '---------------------------------------------------------------------------------------
            Dim ls_interest As String = "select isnull (sum(interest),0) from CF_pb_vismin where " & _
         "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_interest, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_kppayout As String = "select isnull (sum(kp_payout),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_kpsendout As String = "select isnull (sum(kp_sendout),0) from CF_pb_vismin where " & _
          "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_kpsendout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_kpsendoutcomm As String = "select isnull (sum(KP_Sendout_Comm),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_kpsendoutcomm, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_lukat As String = "select isnull (sum(lukat),0) from CF_pb_vismin where " & _
           "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_lukat, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_otherincome As String = "select isnull (sum(OtherIncome),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_otherincome, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_Prenda As String = "select isnull (sum(prenda),0) from CF_pb_vismin where " & _
         "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Prenda, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_telecomms As String = "select isnull (sum(telecomms),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_telecomms, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_souvenirs As String = "select isnull (sum(Souvenirs),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_souvenirs, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_Corp_Sendout As String = "select isnull (sum(Corp_Sendout),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Corp_Sendout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_Corp_Payout As String = "select isnull (sum(Corp_Payout),0) from CF_pb_vismin where " & _
           "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Corp_Payout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_Corp_Comm As String = "select isnull (sum(Corp_Comm),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_Corp_Comm, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_WesternUnionComm As String = "select isnull (sum(WesternUnionComm),0) from CF_pb_vismin where " & _
         "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WesternUnionComm, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_WesternUnionPayout As String = "select isnull (sum(WesternUnionPayout),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WesternUnionPayout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_WesternUnionSendout As String = "select isnull (sum(WesternUnionSendout),0) from CF_pb_vismin where " & _
          "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_WesternUnionSendout, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_FundTransferDebit As String = "select isnull (sum(FundTransferDebit),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_FundTransferDebit, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_FundTransferCredit As String = "select isnull (sum(FundTransferDebit),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_FundTransferCredit, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '--------------------------------------------------------------
            Dim ls_BranchExpense As String = "select isnull (sum(BranchExpense),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_BranchExpense, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            c.DisposeR()
            '----------------------------------------------------------
            Dim ls_OtherExpense As String = "select isnull (sum(OtherExpense),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_OtherExpense, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            Dim ls_nso As String = "select isnull (sum(NSO),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_nso, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_mccr As String = "select isnull (sum(MCCashReceipts),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_mccr, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_mccd As String = "select isnull (sum(MCCashDisbursements),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_mccd, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_racr As String = "select isnull (sum(RACashReceipts),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_racr, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_racd As String = "select isnull (sum(RACashDisbursements),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_racd, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_dfb As String = "select isnull (sum(DepositFromBank),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_dfb, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_wfb As String = "select isnull (sum(WithdrawalFromBank),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_wfb, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            '----------------------------------------------------------
            Dim ls_rts As String = "select isnull (sum(ReturnToSender),0) from CF_pb_vismin where " & _
            "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_rts, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '\-added last 7/26/2011
            '----------------------------------------------------------
            Dim ls_cashover As String = "select isnull (sum(cashover),0) from CF_pb_vismin where " & _
         "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_cashover, rdr, sqlmsg) Then
                If rdr.Read Then
            ...
            '----------------------------------------------------------
            Dim ls_cashshort As String = "select isnull (sum(cashshort),0) from CF_pb_vismin where " & _
       "class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "'"
            If c.ErrorConnectionReading(False) Then Exit Sub
            If Not c.Error_SetRdr(ls_cashshort, rdr, sqlmsg) Then
                If rdr.Read Then
...
" [Telecomms] = " & ld_telecomms_lv & ", [Souvenirs]=" & ld_souvenirs_lv & ", [Corp_Sendout]=" & ld_corpsendout_lv & ", [Corp_Payout]=" & ld_corppayout_lv & ",[Corp_Comm]=" & ld_corpcomm_lv & " ,[WesternUnionComm]=" & ld_westernUnionComm_lv & ", " & _
" [WesternUnionPayout]=" & ld_westernunionPayout_lv & ", [WesternUnionSendout]=" & ld_westernunionsendout_lv & ", [FundTransferDebit]=" & ld_fundtransferdebit_lv & ", [FundTransferCredit]=" & ld_fundtransfercredit_lv & ", " & _
"[BranchExpense]=" & ld_branchexpense_lv & ",[OtherExpense] = " & ld_otherexpense_lv & " ,[NSO] = " & ld_nso_lv & " ,[MCCashReceipts] = " & ld_MCCR_lv & ",[MCCashDisbursements] = " & ld_MCCD_lv & ",[RACashReceipts] = " & ld_RACR_lv & ",[RACashDisbursements] = " & ld_RACD_lv & ",[DepositFromBank] = " & ld_Depositfrombank_lv & " ,[WithdrawalFromBank] = " & ld_withdrawalfrombank_lv & ",[ReturnToSender] = " & ld_returntosender_lv & " ,[cashover] = " & ld_cashover_lv & "  ,[cashshort] = " & ld_cashshort_lv & "  ,[dategenerated] = '" + Now.Date + "'        WHERE class_04 = '" + ls_class04 + "' and transdate = '" + transdate + "' "
    ...
    Private Sub select_per_branch()
        ...
        Dim ls_s As String = "select bedrnr,bedrnm,class_03,class_04 from bedryf order by class_04 asc"
        Dim c As New clsVismin
        If c.Error_Inititalize_INI Then Exit Sub
        If c.ErrorConnectionReading(False) Then Exit Sub
        If Not c.Error_SetRdr(ls_s, rdr, sqlmsg) Then
    ...
    Private Sub select_region()
        ...
        Dim username As String = Nothing
        Dim jobtitle As String = Nothing
        Dim ls_s As String = "select distinct class_03 from bedryf order by class_03 asc"
        Dim c As New clsVismin
        If c.Error_Inititalize_INI Then Exit Sub
        If c.ErrorConnectionReading(False) Then Exit Sub
        If Not c.Error_SetRdr(ls_s, rdr, sqlmsg) Then
    ...
    Private Sub select_area()
        ...
        Dim username As String = Nothing
        Dim jobtitle As String = Nothing
        Dim ls_s As String = "select distinct class_03,class_04 from bedryf order by class_04 asc"
        Dim c As New clsVismin
        If c.Error_Inititalize_INI Then Exit Sub
        If c.ErrorConnectionReading(False) Then Exit Sub
        If Not c.Error_SetRdr(ls_s, rdr, sqlmsg) Then
            ...
            c.DisposeR()
        End If
        Dim tryarea As Integer = ListviewArea.Items.Count - 1
...
" where transdate = '" + transdate + "' and branchcode = '" + gs_bcode + "'"
        Dim c As New clsData
        If c.Error_Inititalize_INI Then Exit Sub
        If c.ErrorConnectionReading(False) Then Exit Sub
        If Not c.Error_SetRdr(ls_s, rdr, sqlmsg) Then
...
" where transdate = '" + transdate + "' and class_03 = '" + gs_class03 + "'"
        Dim c As New clsData
        If c.Error_Inititalize_INI Then Exit Sub
        If c.ErrorConnectionReading(False) Then Exit Sub
        If Not c.Error_SetRdr(ls_s, rdr, sqlmsg) Then
...
" where transdate = '" + transdate + "' "
    ...
    Private Sub tmr_Start_Tick(ByVal sender As Object, ByVal e As System.EventArgs) Handles tmr_Start.Tick
        Dim starttime As DateTime
        starttime = Now.ToShortTimeString
        If ctr = 0 Then
            If starttime = "12:00:00 AM" Then
